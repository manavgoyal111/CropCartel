...
const Vegetable = ({ products }) => {
	return (
		<div className="min-h-screen">
			<Head>
				<title>Vegetable | CropCartel</title>
			</Head>
				...
				<div className="container px-5 py-24 mx-auto">
					<div className="flex flex-wrap -m-4 justify-center mx-5">
						{Object.keys(products).length === 0 && (
							<p>
								Sorry, All the Vegetables are currently out of stock. New stock coming
								soon. Stay tuned!
							</p>
						)}
...
export async function getServerSideProps(context) {
	...
	for (let item of products) {
		if (item.title in vegetables) {
			if (!vegetables[item.title].color.includes(item.color) && item.availableQty > 0) {
				vegetables[item.title].color.push(item.color);
			}
			if (!vegetables[item.title].size.includes(item.size) && item.availableQty > 0) {
				vegetables[item.title].size.push(item.size);
			}
		} else {
			vegetables[item.title] = JSON.parse(JSON.stringify(item));
			if (item.availableQty > 0) {
				vegetables[item.title].color = [item.color];
				vegetables[item.title].size = [item.size];
			} else {
				vegetables[item.title].color = [];
				vegetables[item.title].size = [];
			}
		}
	}

	return {
		props: { products: JSON.parse(JSON.stringify(vegetables)) },
	};
}
