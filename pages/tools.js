				...
				<div className="container px-5 py-24 mx-auto">
					<div className="flex flex-wrap -m-4 justify-center mx-5">
						{Object.keys(products).length === 0 && (
							<p>
								Sorry, All the Tools are currently out of stock. New stock coming
								soon. Stay tuned!
							</p>
						)}
...
export async function getServerSideProps(context) {
	...
	let products = await Product.find({ category: "tool" });
	let tools = {};
	...
	return {
		props: { products: JSON.parse(JSON.stringify(tools)) },
	};
}
