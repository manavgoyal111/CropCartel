				...
				<div className="container px-5 py-24 mx-auto">
					<div className="flex flex-wrap -m-4 justify-center mx-5">
						{Object.keys(products).length === 0 && (
							<p>
								Sorry, All the Kurtis are currently out of stock. New stock coming
								soon. Stay tuned!
							</p>
						)}
...
export async function getServerSideProps(context) {
	...
	for (let item of products) {
		if (item.title in kurtis) {
			if (!kurtis[item.title].color.includes(item.color) && item.availableQty > 0) {
				kurtis[item.title].color.push(item.color);
			}
			if (!kurtis[item.title].size.includes(item.size) && item.availableQty > 0) {
				kurtis[item.title].size.push(item.size);
			}
		} else {
			kurtis[item.title] = JSON.parse(JSON.stringify(item));
	...
	return {
		props: { products: JSON.parse(JSON.stringify(kurtis)) },
	};
}
