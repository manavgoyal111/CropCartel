...
const Contact = () => {
	...
	const { name, email, contact, message } = inputVal;
	const [disabled, setDisabled] = useState(true);

	const form = useRef();

	useEffect(() => {
		if (name.length > 3 && email.length > 5 && contact.length > 9 && message.length > 3) {
			setDisabled(false);
		} else {
			setDisabled(true);
		}
	...
	return <div className="min-h-screen">
		<Head>
			<meta
				name="description"
				content="CropCartel - Contact information"
			/>
			<title>Contact | CropCartel</title>
		</Head>
		...
		<section className="text-gray-600 body-font relative">
			<div className="container px-5 py-24 mx-auto flex sm:flex-nowrap flex-wrap">
				<div className="lg:w-2/3 md:w-1/2 bg-gray-300 rounded-lg overflow-hidden sm:mr-10 p-10 flex items-end justify-start relative">
					...
					<div className="bg-white relative flex flex-wrap py-6 rounded shadow-md">
						...
						<div className="lg:w-1/2 px-6 mt-4 lg:mt-0">
							...
							<a className="text-indigo-500 leading-relaxed">contact@CropCartel</a>
				...
				<form ref={form} className="lg:w-1/3 md:w-1/2 bg-white flex flex-col md:ml-auto w-full md:py-8 mt-8 md:mt-0" onSubmit={sendEmail}>
					...
					<div className="relative mb-4">
						<label htmlFor="contact" className="leading-7 text-sm text-gray-600">Phone</label>
						<input onChange={handleChange} value={contact} type="number" id="contact" name="contact" className="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
					</div>
					<div className="relative mb-4">
						<label htmlFor="message" className="leading-7 text-sm text-gray-600">Message</label>
						<textarea onChange={handleChange} value={message} id="message" name="message" className="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out"></textarea>
					</div>
					<button disabled={disabled} className="text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg disabled:bg-indigo-300">Button</button>
