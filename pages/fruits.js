...
const Fruit = ({ products }) => {
	return (
		<div className="min-h-screen">
			<Head>
				<title>Fruit | CropCartel</title>
			</Head>
				...
				<div className="container px-5 py-24 mx-auto">
					<div className="flex flex-wrap -m-4 justify-center mx-5">
						{Object.keys(products).length === 0 && (
							<p>
								Sorry, All the Fruits are currently out of stock. New stock coming
								soon. Stay tuned!
							</p>
						)}
...
export async function getServerSideProps(context) {
	...
	let products = await Product.find({ category: "fruit" });
	let fruits = {};
	for (let item of products) {
		if (item.title in fruits) {
			if (!fruits[item.title].color.includes(item.color) && item.availableQty > 0) {
				fruits[item.title].color.push(item.color);
			}
			if (!fruits[item.title].size.includes(item.size) && item.availableQty > 0) {
			...
			if (item.availableQty > 0) {
				...
				fruits[item.title].color = [];
				fruits[item.title].size = [];
			}
		}
	}

	return {
		props: { products: JSON.parse(JSON.stringify(fruits)) },
	};
}
